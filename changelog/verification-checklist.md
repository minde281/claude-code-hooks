...
| 4B | Settings Hook Structure | For each hook across both settings files (`.claude/settings.json`, `install/settings.json`), verify: timeout value (5000 for all except Setup which is 30000), async is true, `once` flag only on PreCompact/SessionStart/SessionEnd, statusMessage matches hook name | field-level | settings.json vs install/settings.json | 2026-02-24 | Wrong timeout or missing once flag would cause behavioral bugs not caught by name/count checks |
